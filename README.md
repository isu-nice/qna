...
#### 회원을 등록하는 기능
- [x] 회원을 등록할 수 있다.
  - [x] 회원은 회원의 상태 값이 필요하다.
    - MEMBER_ACTIVE - 활동중
    - MEMBER_SLEEP - 휴면 상태
    - MEMBER_QUIT - 탈퇴 상태
  - [x] 회원의 권한에 대한 상태 값이 필요하다.
    - GENERAL_MEMBER - 일반 회원
    - ADMIN - 관리자
    - Spring Security를 학습하지 않았으므로 별도의 회원 로그인 인증 과정은 없는 것으로 가정한다. (Spring Security를 적용해도 무방하다)
    - [x]  `admin@gmail.com`만 관리자의 역할을 하며, 애플리케이션에 상수 형태로 고정해서 사용해도 무방하다.
- [x] 회원 등록 날짜가 생성되어야 한다.
- [x] 회원이 등록되면 회원에게 등록 완료 안내 메일을 전송한다.

#### 회원 정보를 수정하는 기능
- [x] 회원 정보를 수정할 수 있다.
  - [x] 수정할 정보만 입력하면 업데이트 된다.

#### 회원을 조회하는 기능
- [x] 등록된 회원 중 특정 회원을 조회할 수 있다.
- [x] 모든 등록된 회원을 조회할 수 있다.

#### 회원을 삭제하는 기능
- [x] 등록된 회원을 삭제할 수 있다.

#### 회원이 커피 매장에 질문을 등록하는 기능
- [x]  질문은 회원(고객)만 등록할 수 있다.
- [x]  질문 등록시 등록 날짜가 생성 되어야 한다.
- [x]  질문은 질문의 상태 값이 필요하다
    - QUESTION_REGISTERED- 질문 등록 상태
    - QUESTION_ANSWERED - 답변 완료 상태
    - QUESTION_DELETED - 질문 삭제 상태
    - QUESTION_DEACTIVATED - 질문 비활성화 상태: 회원 탈퇴 시, 질문 비활성화 상태
- [x]  질문 등록 시, 초기 상태 값은 QUESTION_REGISTERED 이어야 한다.
- [x]  질문 제목과 내용은 필수입력 사항이다.
- [x]  질문은 비밀글과 공개글 둘 중에 하나로 설정되어야 한다.
    - PUBLIC - 공개글 상태
    - SECRET - 비밀글 상태
